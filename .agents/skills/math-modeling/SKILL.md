...
- 为当前题目建立独立的 `acceptance/` 验收材料目录，单独存放验收标准、验收报告和验收记录
- 依据题型和优秀论文参考建立验收标准，先完成当前题验收，再决定是否进入下一题
- 若用户根据图表、分布、约束、论文展示效果提出反馈，必须先修改当前题代码并重新验收，通过后再进入下一题
- 创建README文档说明项目
...
- `README.md` - 项目说明文档
- 生成的可视化图表
- `acceptance/问题X_验收标准.md` - 当前题的验收标准
- `acceptance/问题X_验收报告.md` - 当前题的验收报告
...
- **`references/文献检索与引用规范.md`** - 中英文文献检索、权威筛选、正文短引用与参考文献记录规范
...
6. **多版思路迭代与版本控制**：当用户提供多版建模思路、要求对比当前报告并融合优化，或要求使用 Git 管理报告版本时，先阅读 `references/建模思路迭代与版本控制.md`。模型取舍对比只在聊天区说明；写入报告时只保留最终融合方案。Git 提交信息和标签使用简体中文，标签示例：`建模思路v1`、`建模思路v2`。
7. **文献检索与引用**：涉及参考文献、模型依据或论文写作时，先阅读 `references/文献检索与引用规范.md`。报告中只写最终采用文献和正文短引用，不写检索筛选过程；中文文献无法验证时必须先向用户索要可验证材料，不能跳步或编造。
8. **验收反馈闭环**：当用户要求检查图表合理性、数据分布、约束满足、模型结果是否可用于论文，或要求“一个问题一个问题改”时，先为当前题建立验收标准，再生成验收报告；若结论为“不通过”或“需复核”，必须依据验收反馈继续修改当前题，不能跳到下一题。
