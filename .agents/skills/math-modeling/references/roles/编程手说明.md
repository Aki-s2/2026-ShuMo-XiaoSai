...
- 每个代码项目目录下单独建立 `acceptance/` 文件夹。
- 验收标准、验收报告、验收记录统一放入 `acceptance/`，不要与 `results/` 混放。
- `results/` 只放数值结果和导出表格，`figures/` 只放图表，`acceptance/` 只放验收材料。

**验收标准建立要求**：

- 优化类问题：重点检查模型可行性、算法有效性、基准对比、敏感性分析、图表支撑，不强制套用正态性检验或显著性检验。
- 预测类、统计类问题：根据题型补充误差指标、拟合优度、显著性检验等。
- 验收标准必须结合题目类型、建模方法和优秀论文常见写法，不允许只做工程层面的“跑通检查”。

**验收反馈流程**：

1. 先生成 `acceptance/问题X_验收标准.md`。
2. 运行代码并生成 `acceptance/问题X_验收报告.md`。
3. 根据验收报告判断结论：`通过`、`需复核`、`不通过`。
4. 若用户对图表分布、约束满足、结果美观、论文可解释性提出反馈，视为新的验收输入。
5. 对当前题直接修改原代码并重新运行，更新验收报告。
6. 当前题未达到可接受状态前，不进入下一题。
...
| **文档记录** | 记录每次改进的内容和原因 |
| **验收驱动** | 每次改动后必须重新运行当前题并更新验收报告，用验收结论驱动下一轮修改 |
